...
- 어절 단위 상품명 줄바꿈, 좌우 너비 조절, 작은 화면에서 상하 배치, 하단 고정 합계·확정 버튼
- 수량 입력 후 Enter로 구성품 추가, 행 더블클릭으로 수량 변경, 입력 누락 위치 안내
- F1 검색형 사용 안내와 별도 HTML 설명서. 설치파일·포터블·단일 EXE에서 오프라인 이용
...
- 이전 작업 검색·이어하기·완료, 재실행 시 진행 작업 복구
- 진행 중인 작업의 **어느 박스든** 확정 취소·정정. 원본·사유·처리자를 보존하고 정정은 재확정 시 한 번에 적용
- 정정은 박스번호를 그대로 물려받는다. 취소·수량 변경은 뒤 번호를 당겨 `#1`부터 빈틈없이 유지하고, 번호가 밀린 박스는 재부착 목록으로 안내
- 숫자 입력칸은 커서가 들어오면 기존 값을 선택해 그대로 덮어쓴다
- 확정 직전 현재 로컬 상품DB로 사용중지·변경 여부 재검증, 박스별 사용 버전 기록
- 출고건 단위 Excel 포장실적
- 포장 실적 자동 백업: 공유 폴더에 DB 사본과 일자별 CSV를 작업대별 폴더로 저장
- 공유 폴더 설정 없이 로컬 자동 백업, 시간별 DB 사본, 검증 후 화면에서 복원
...
5. `설정·관리 > 라벨 설정`에서 프린터와 크기를 지정한다. 안내줄이 초록인지 확인하고 `테스트 라벨 출력`으로 검증한다.
6. `설정·관리 > 자동 백업 설정`에서 공유 폴더를 지정한다. 미설정이어도 포장기록은 로컬에 자동 백업된다. PC 고장 대비 외부 사본과 상품 마스터 사본은 공유 폴더 설정이 필요하다.
...
- `BeyondPack-<버전>-User-Guide.html`: 검색·인쇄 가능한 오프라인 사용 안내. 시작 메뉴에서도 열 수 있음
