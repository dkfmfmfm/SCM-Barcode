...
- 출고건 작업 현황 실시간 누적 표시(FNSKU 기준)와 지난 박스 재출력
...
6. `설정·관리자 도구 > 자동 백업 설정`에서 공유 폴더를 지정한다. 지정하지 않으면 포장 실적이 이 PC에만 남는다.
...
포장 실적은 `자동 백업 설정`의 위치에 `<PC이름>\packaging.db`와 `<PC이름>\packing-YYYYMMDD.csv`로 함께 복사된다.
