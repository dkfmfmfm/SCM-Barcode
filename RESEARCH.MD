...
이 문서는 프로그램의 설계 판단, 기술 선택, 보안·데이터 기준과 중장기 확장 방향을 결정하는 공식 연구 기준이다.

- 현재 1차 개발의 구체적인 필수 기능·테스트·인수 기준은 `PLAN.MD`를 따른다.
- 이 문서의 WMS 기능 전체를 1차 BeyondPack 개선에 한꺼번에 구현하지 않는다.
- `PLAN.MD`와 충돌하면 현재 개발에는 `PLAN.MD`가 우선하며, 중장기 구조 변경에는 본 문서의 원칙을 검토한다.
- 요구사항이나 아키텍처가 변경되면 관련 코드, `PLAN.MD`, `RESEARCH.MD`를 같은 PR에서 갱신한다.
- 중요한 구현 결정은 근거와 영향범위를 PR 또는 ADR에 남긴다.

### 2026-08-11 확정 결정: 국가 선택형 복합키

실데이터 검증 결과 동일 FNSKU가 서로 다른 국가에 존재하는 정상 사례가 확인되었다. 따라서 FNSKU 단독 고유값 가정은 폐기하고 `FNSKU|CountryCode`를 상품 조회·동기화·중복검사의 공식 키로 사용한다.

- SharePoint `FNSKU`는 중복을 허용하되 인덱스를 유지한다.
- `LookupKey`는 한 줄 텍스트 필드로 저장하고 고유값을 적용한다.
- 작업자는 포장 시작 시 국가를 선택하며, 스캔은 로컬 캐시에서 복합키로 즉시 조회한다.
- 동일 FNSKU의 국가별 변형은 정상으로 수용하고, 동일 FNSKU+국가만 데이터 오류로 차단한다.
- 박스 구성 중 국가 변경을 막아 잘못된 국가의 SKU·품목코드·라벨이 섞이는 것을 예방한다.
- 상품 캐시와 포장실적에는 국가코드·국가명 스냅샷을 남겨 사후 추적성을 확보한다.
...
현장 연결성과 운영 난이도를 다시 평가한 결과, 단기 상품 마스터의 상시 원본은 Google Sheet로 확정한다. 이는 v1.4의 단순 CSV 조회로 되돌아가는 것이 아니라 다음 통제를 추가한 개선안이다.

- Google Sheet 주소는 EXE에 하드코딩하지 않고 PC 로컬 설정에 저장
- 프로그램 시작과 F2 실행 시 해당 탭 CSV 자동 다운로드
- `FNSKU+CountryCode` 복합키, 필수 열, 상품 수 급감, 버전·스키마 검증
- 새 SQLite를 별도로 생성·검증한 뒤 원자 교체하고 직전 DB 보관
- 인터넷·공유·CSV 오류 시 검증된 기존 DB로 계속 작업
- Google Sheet 장애 시 동일 스키마의 Excel을 관리자가 비상 적용
- 스캔 시에는 네트워크를 호출하지 않고 로컬 SQLite만 조회

따라서 제거 대상은 Google Sheet 자체가 아니라 **하드코딩된 공개 주소, 무검증 다운로드, 오프라인 캐시 부재**다. 원가·매출·개인정보는 Google Sheet에 넣지 않으며, 해당 정보가 필요해지면 인증형 API 또는 WMS ProductSource로 전환한다.
